...
function modelSortControl(categoryIndex, modelIndex) {
    $("#arrangeBtn").click(function () {
        // $.get('/manage/models/sort/' + categoryIndex + '/' + modelIndex, function (res) {
...
function getConponents(categoryIndex, modelIndex) {
    if (getQueryString("sort")) {
        $.get('/manage/models/' + categoryIndex + '/' + modelIndex + '?sort=true', function (res) {
            console.log(res);
            var objRes = res;
        ...
        $.get('/manage/models/' + categoryIndex + '/' + modelIndex, function (res) {
            console.log(res);
            var objRes = res;
            var objLen = objRes.length;
            getLen(objLen);
...
function getCategory() {
    $.get('/manage/model/find-category', function (res) {
        var objRes = res;
        optionBuild(objRes);
    });
}
...
function getLen(len) {
    $.get("/manage/models/num", function (res) {
...
function compDelete(cateIndex, modelIndex, compIndex) {
    $.get('/manage/comp-rm/' + cateIndex + '/' + modelIndex + '/' + compIndex, function (res2) {
        alert('删除成功！');
...
function compModify(compObjectId, cateIndex2, modelIndex2, compIndex2) {
    ...
    $.post("/manage/mod-comp/" + cateIndex2 + "/" + modelIndex2 + "/" + compIndex2,
        {
        ...
        function (res) {
            ...
            if (status == 0) {
                alert('已更新子部件')
                window.location.href = '/manage?category_index=' + cateIndex2 + '&model_index=' + modelIndex2;
            }
        });
}
        ...
        if (f.files.length > 0) {
            ...
            file.save().then(function (theFile) {
                new Promise((resolve, reject) => {
                    ...
                    $.ajax({
                        url: '/manage/mod-comp-gcode/' + cateIndex3 + '/' + modelIndex3 + '/' + compIndex3 + '/' + compObjectId,
                        type: 'POST',
                        cache: false,
                        data: value,
                        processData: false,
...
function addcompInfo(index1, index2) {
    $('#add-btn').click(function () {
        $.ajax({
            url: '/manage/add-comp/' + index1 + '/' + index2,
            type: 'POST',
            cache: false,
...
function getCostInfo(cateIndex, modelIndex) {
    $.get('/manage/cost-get/' + cateIndex + '/' + modelIndex, function (costInfo) {
